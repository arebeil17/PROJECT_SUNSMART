...
boolean Adafruit_GPS::parse(char *nmea) {
  ...
  //Serial.printlnf("[GPS_INTERNAL] nmea: %s", nmea);
  //Serial.printlnf("[GPS_INTERNAL] checksum present: %c", nmea[strlen(nmea)-4]);
  //Serial.printlnf("[GPS_INTERNAL] checksum: %c", nmea[strlen(nmea)-3]);
  if (nmea[strlen(nmea)-4] == '*') {
    ...
    if (sum != 0) {
      // bad checksum :(
      //Serial.printlnf("[GPS_INTERNAL] BAD CHECKSUM (%i)!", sum);
      return false;
    }
  }
  int32_t degree;
  ...
  if (strstr(nmea, "$GPGGA")) {
    //Serial.println("[GPS_INTERNAL] FOUND $GPGGA!!!");
    // found GGA
    char *p = nmea;
    // get time
    p = strchr(p, ',')+1;
